...
public class QuotCombCalculator {
    ...
    private HostingQuotationComb mOutputCombQuotationItem = new HostingQuotationComb();
    ...
    public QuotCombCalculator(HostingComposeGraph composeGraph) throws UnsupportedEncodingException {
        ...
        mOutputCombQuotationItem.setComposeGraphId(composeGraph.getComposeGraphId());
        mOutputCombQuotationItem.setCombItem(new QuotationItem());
        mOutputCombQuotationItem.getCombItem().setPlatform("COMB");
        mOutputCombQuotationItem.getCombItem().setContractSymbol(String.valueOf(getComposeGraphId()));
    }
    ...
    private void calculate() {
        ...
        if (bidPrice != null) {
            mOutputCombQuotationItem.getCombItem().setBidPrice(Arrays.asList(bidPrice));
        } else {
            mOutputCombQuotationItem.getCombItem().unsetBidPrice();
        }
        
        Double askPrice = calculateAskPrice();
        if (askPrice != null) {
            mOutputCombQuotationItem.getCombItem().setAskPrice(Arrays.asList(askPrice));
        } else {
            mOutputCombQuotationItem.getCombItem().unsetAskPrice();
        }
        
        Long bidQty = calculateBidQty();
        if (bidQty != null) {
            mOutputCombQuotationItem.getCombItem().setBidQty(Arrays.asList(bidQty));
        } else {
            mOutputCombQuotationItem.getCombItem().unsetBidQty();
        }
        
        Long askQty = calculateAskQty();
        if (askQty != null) {
            mOutputCombQuotationItem.getCombItem().setAskQty(Arrays.asList(askQty));
        } else {
            mOutputCombQuotationItem.getCombItem().unsetAskQty();
        }
        
        Double lastPrice = calculateLastPrice();
        if (lastPrice != null) {
            mOutputCombQuotationItem.getCombItem().setLastPrice(lastPrice);
        } else {
            mOutputCombQuotationItem.getCombItem().unsetLastPrice();
        }
        
        mOutputCombQuotationItem.getCombItem().setUpdateTimestampMs(System.currentTimeMillis());
        mOutputCombQuotationItem.getCombItem().setRaceTimestampMs(mOutputCombQuotationItem.getCombItem().getUpdateTimestampMs());
        
        mOutputCombQuotationItem.setLegItems(new ArrayList<QuotationItem>(mContractQuotationItemLastest.values()));
    }
    ...
    public HostingQuotationComb getCombQuotationItem() {
        return new HostingQuotationComb(mOutputCombQuotationItem);
    }
}
