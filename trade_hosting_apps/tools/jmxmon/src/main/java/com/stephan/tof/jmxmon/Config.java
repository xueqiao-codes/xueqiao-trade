...
public class Config {
	...
	private String[] jmxProcessNames;
	...
	public void init(String configPath) throws ConfigurationException, IOException{
		...
		jmxProcessNames = config.getStringArray("jmx.processnames");
		
		if (jmxPorts.length != jmxProcessNames.length) {
		    throw new IllegalArgumentException("jmx.processnames not match jmx.ports config, jmxPorts.length="
		            + jmxPorts.length + ", jmxProcessNames.length=" + jmxProcessNames.length);
		}
	...
	public String[] getJmxProcessNames() {
	    return jmxProcessNames;
	}
