...
private:
    ...
    void handleAuthenticateResp(const std::shared_ptr<CTPAuthenticateResp>& authenticate_rsp);
    ...
    void handleAuthenticateFailed(int api_ret_code);
    void handleAuthenticateSuccess(const std::shared_ptr<CThostFtdcRspAuthenticateField>& authenticate_rsp);
    ...
    std::string app_id_info_;
    std::string auth_code_info_;

    std::shared_ptr<CTPLoginHandler> login_handler_;
    std::shared_ptr<CTPAuthenticateHandler> authenticate_handler_;

    int32_t max_order_ref_ = 0;
    std::shared_ptr<CThostFtdcRspUserLoginField> login_rsp_;
    std::shared_ptr<CThostFtdcRspAuthenticateField> authenticate_rsp_;
};
