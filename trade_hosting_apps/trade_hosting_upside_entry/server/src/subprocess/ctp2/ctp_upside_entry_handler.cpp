...
bool CTPUpsideEntryHandler::init(const std::shared_ptr<BrokerAccessEntry>& broker_access_entry) {
	...
	//找由雪橇设置的全局的期货公司授权码
	auto it_global_CTP_STS_APPID = broker_access_entry->customInfoMap.find("CTP_STS_APPID");
	auto it_global_CTP_STS_AUTHCODE = broker_access_entry->customInfoMap.find("CTP_STS_AUTHCODE");
	//找由用户设置的私有的期货公司授权码
	auto it_user_CTP_STS_APPID = trade_account_->accountProperties.find("CTP_STS_APPID");
	auto it_user_CTP_STS_AUTHCODE = trade_account_->accountProperties.find("CTP_STS_AUTHCODE");
	std::string ctp_sts_appid = "";
	std::string ctp_sts_authcode = "";

	//CTP授权码的使用原则是，如果个人有设置，就用个人的，否则，就看看雪橇的全局有没有设置
	if (it_global_CTP_STS_APPID != broker_access_entry->customInfoMap.end() && it_global_CTP_STS_AUTHCODE != broker_access_entry->customInfoMap.end() )
	{
		if(!it_global_CTP_STS_APPID->second.empty() && !it_global_CTP_STS_AUTHCODE->second.empty())
		{
			ctp_sts_appid = it_global_CTP_STS_APPID->second;
			ctp_sts_authcode = it_global_CTP_STS_AUTHCODE->second;
		}
	}
	if (it_user_CTP_STS_APPID != trade_account_->accountProperties.end() && it_user_CTP_STS_AUTHCODE != trade_account_->accountProperties.end() )
	{
		if(!it_user_CTP_STS_APPID->second.empty() && !it_user_CTP_STS_AUTHCODE->second.empty())
		{
			ctp_sts_appid = it_user_CTP_STS_APPID->second;
			ctp_sts_authcode = it_user_CTP_STS_AUTHCODE->second;
		}
	}

	login_manager_.reset(new CTPLoginManager(dispatcher_, trade_account_, it->second, ctp_sts_appid, ctp_sts_authcode));
