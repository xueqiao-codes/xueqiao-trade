...
public class StatArchiveItemTable extends TableHelper<StatClosedPositionItem> implements IDBTable {

	private final static String TABLE_NAME = "stat_archive_item";
	...
	public StatArchiveItemTable(Connection conn) {
		super(conn);
	}
	...
	public int insert(StatClosedPositionItem item) throws SQLException {
		...
		pf.addLong(F_CLOSED_TIMESTAMP_MS, item.getClosedTimestampMs());
	 ...
	 * 查询
	 */
	public PageResult<StatClosedPositionItem> getArchiveClosedPositionItemPage(QueryStatArchiveItemOption queryOption, IndexedPageOption pageOption) throws SQLException {
		...
		if (queryOption != null) {
			if (queryOption.isSetClosedId() && queryOption.getClosedId() > 0) {
			...
			if (queryOption.isSetArchivedDateTimestampMs()) {
				qryBuilder.addFieldCondition(SqlQueryBuilder.ConditionType.AND, F_ARCHIVED_DATE_TIMESTAMP_MS + "=?", queryOption.getArchivedDateTimestampMs());
			} else if (queryOption.isSetArchiveStartDateTimestampMs() && queryOption.isSetArchiveEndDateTimestampMs()) {

				long startDateZeroTimestamps = TimeUtil.getCurrentDateZeroMillis(queryOption.getArchiveStartDateTimestampMs());
				long endDateZeroTimestamps = TimeUtil.getCurrentDateZeroMillis(queryOption.getArchiveEndDateTimestampMs());
	...
	public void onUpgradeOneStep(IDBOperator operator, int targetVersion) throws SQLException {
		if (1 == targetVersion) {
			...
			createSqlBuilder.append("CREATE TABLE IF NOT EXISTS ").append(getTableName()).append("(")
				...
				.append("INDEX(").append(F_ARCHIVED_DATE_TIMESTAMP_MS).append(")")
