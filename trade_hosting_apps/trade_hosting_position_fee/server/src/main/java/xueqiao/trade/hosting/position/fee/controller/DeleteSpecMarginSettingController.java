...
public class DeleteSpecMarginSettingController implements IController {

    private long subAccountId;
    private long commodityId;

    public DeleteSpecMarginSettingController(long subAccountId, long commodityId) {
    ...
    public void doDelete() throws ErrorInfo {
        new DBTransactionHelper<Void>(PositionFeeDB.Global()) {
            ...
            public void onUpdate() throws ErrorInfo, Exception {
                new XQSpecMarginSettingsTable(getConnection()).delete(subAccountId, commodityId);
                /*
                 * 删除商品持仓费率设置时，需要更新雪橇用户的持仓费率，偏移值由原商品持仓费率偏移值变为通用值
                 * */
                XQContractMarginHandler.updateXQContractMarginToGeneralSettingDataType(getConnection(), subAccountId, commodityId);
            }
