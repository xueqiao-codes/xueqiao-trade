...
public class XQSpecMarginSettingsTable extends TableHelper<XQSpecMarginSettings> implements IDBTable {

    private final static String TABLE_NAME = "txq_spec_margin_settings";
    ...
    public XQSpecMarginSettingsTable(Connection conn) {
        super(conn);
    }

    public void insert(XQSpecMarginSettings settings) throws SQLException {
    ...
    public void update(XQSpecMarginSettings settings) throws SQLException {
        long currenMillis = System.currentTimeMillis();
        PreparedFields pf = getPreparedFields(settings);
        pf.addLong(F_LASTMODITY_TIMESTAMP_MS, currenMillis);
    ...
    public XQSpecMarginSettings query(long subAccountId, long commodityId) throws SQLException {
    ...
    public List<XQSpecMarginSettings> queryUnsyncList(int pageIndex, int pageSize) throws SQLException {
        SqlQueryBuilder qryBuilder = super.prepareSqlQueryBuilder();
    ...
    public XQSpecMarginSettingPage query(QueryXQSpecSettingOptions queryOptions, IndexedPageOption pageOption) throws SQLException {
        SqlQueryBuilder qryBuilder = super.prepareSqlQueryBuilder();
        ...
        XQSpecMarginSettingPage resultPage = new XQSpecMarginSettingPage();
        if (pageOption.isNeedTotalCount()) {
            resultPage.setTotalNum(super.getTotalNum(qryBuilder));
        }
        resultPage.setXQSpecMarginSettingList(super.getItemList(qryBuilder));
        return resultPage;
    }

    private PreparedFields getPreparedFields(XQSpecMarginSettings settings) {
        PreparedFields pf = new PreparedFields();
    ...
    @Override
    public XQSpecMarginSettings fromResultSet(ResultSet rs) throws Exception {
        CommodityInfo commodityInfo = new CommodityInfo();
        commodityInfo.setExchangeMic(rs.getString(F_EXCHANGE_MIC));

        XQSpecMarginSettings xqSpecMarginSettings = new XQSpecMarginSettings();
        xqSpecMarginSettings.setSubAccountId(rs.getLong(F_SUB_ACCOUNT_ID));
//        xqSpecMarginSettings.setExchangeMic(rs.getString(F_EXCHANGE_MIC));
        xqSpecMarginSettings.setCommodityId(rs.getLong(F_COMMODITY_ID));
        xqSpecMarginSettings.setType(FeeCalculateType.findByValue(rs.getInt(F_CALCULATE_TYPE)));
        xqSpecMarginSettings.setCreateTimestampMs(rs.getLong(F_CREATE_TIMESTAMP_MS));
        xqSpecMarginSettings.setLastmodifyTimestampMs(rs.getLong(F_LASTMODITY_TIMESTAMP_MS));
        xqSpecMarginSettings.setIsSync(rs.getBoolean(F_IS_SYNC));
        ...
        xqSpecMarginSettings.setMarginDelta(marginDelta);
        xqSpecMarginSettings.setCommodityInfo(commodityInfo);
        return xqSpecMarginSettings;
    }
